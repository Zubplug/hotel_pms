...
for (const file of sourceFiles) {
      ...
      for (const dec of allVars) {
          if (dec.getInitializer()?.getText().includes('session.user')) {
              ...
              if (nameNode.getKind() === SyntaxKind.ObjectBindingPattern) {
                  ...
                  const remaining = elements.filter(e => e.getPropertyNameNode()?.getText() !== 'propertyId' && e.getName() !== 'propertyId' && e.getPropertyNameNode()?.getText() !== 'outletId' && e.getName() !== 'outletId');
  ...
  if (text.match(/where:\s*{\s*outletId\s*}/)) {
     text = text.replace(/where:\s*{\s*outletId\s*}/g, 'where: { outletId: { in: ctx.outletIds as string[] } }');
     textChanged = true;
  }
  if (text.match(/where:\s*{\s*id,\s*outletId\s*}/)) {
     text = text.replace(/where:\s*{\s*id,\s*outletId\s*}/g, 'where: { id, outletId: { in: ctx.outletIds as string[] } }');
     textChanged = true;
  }
  ...
  text = text.replace(/data:\s*{\s*outletId\s*,/g, 'data: { outletId: (typeof reqOutletId !== "undefined" ? reqOutletId : ctx.outletIds[0]),');
  text = text.replace(/data:\s*{\s*([^,}]+,)*\s*outletId\s*,/g, (match) => match.replace('outletId', 'outletId: (typeof reqOutletId !== "undefined" ? reqOutletId : ctx.outletIds[0])'));
  ...
  for (const stmt of varStmts) {
      ...
      if (stmt.getText().includes('const reqOutletId')) {
          reqOutletCount++;
          if (reqOutletCount > 1) {
              stmt.replaceWithText(stmt.getText().replace('const reqOutletId', 'reqOutletId'));
          } else {
              stmt.replaceWithText(stmt.getText().replace('const reqOutletId', 'let reqOutletId'));
          }
      }
  }
