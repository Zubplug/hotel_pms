...
const globs = [
    'apps/web/src/app/api/v1/rooms/**/*.ts',
    'apps/web/src/app/api/v1/properties/**/*.ts',
    'apps/web/src/app/api/v1/room-types/**/*.ts',
    'apps/web/src/app/api/v1/buildings/**/*.ts'
];
...
for (const file of sourceFiles) {
  ...
  for (const func of funcs) {
      ...
      if (sessionDecl) {
          ...
          if (stmt) {
              ...
              if (block) {
                  // Find the authorization check `if (!session?.user) ...`
                  ...
                  for (let i = insertIdx; i < stmts.length; i++) {
                      if (stmts[i].getKind() === SyntaxKind.IfStatement && stmts[i].getText().includes('session')) {
                          insertIdx = i + 1;
                          break; // Insert after the auth check
                      }
                  }
  ...
  // 3. Text replacements for Prisma queries
  ...
  // Safely map body propertyId check
  ...
  // Clean up duplicate reqPropertyId declarations if there are multiple await req.json() calls in same block
  // Just use let reqPropertyId
  
  if (text.match(/[\s,{]propertyId\s*[,}]/)) {
     text = text.replace(/([,{]\s*)propertyId(\s*[,}])/g, '$1propertyId: (typeof reqPropertyId !== "undefined" ? reqPropertyId : ctx.propertyIds[0])$2');
     textChanged = true;
  }
  if (text.match(/[\s,{]sessionPropertyId\s*[,}]/)) {
     text = text.replace(/([,{]\s*)sessionPropertyId(\s*[,}])/g, '$1sessionPropertyId: ctx.propertyIds[0]$2');
     textChanged = true;
  }
  ...
  // Replace let duplicate declarations
