...
export default async function CashExpensesPage() {
  ...
  return <div className="min-h-full bg-[#08111f]"><div className="relative overflow-hidden bg-gradient-to-r from-[#0b1120] via-[#101d34] to-[#0b1120] px-6 py-8 sm:px-8"><div className="pointer-events-none absolute -right-20 -top-24 h-64 w-64 rounded-full bg-indigo-500/20 blur-3xl" /><div className="relative mx-auto flex max-w-[1440px] items-center gap-3"><div className="flex h-11 w-11 items-center justify-center rounded-xl bg-white/10 text-indigo-200"><ReceiptText className="h-5 w-5" /></div><div><p className="mb-1 text-xs font-semibold uppercase tracking-[0.18em] text-indigo-300">Cash disbursement control</p><h1 className="text-2xl font-bold tracking-tight text-white">Cash expenses</h1><p className="mt-1 text-sm text-slate-400">Record, approve, and pay controlled expenses from the General Cashier Safe.</p></div></div></div><div className="mx-auto max-w-[1440px] px-5 py-7 sm:px-8"><ExpenseWorkspace propertyId={propertyIds.length === 1 ? propertyIds[0] : ''} expenses={serialized} categories={categories.map(item => ({ id: item.id, code: item.code, name: item.name }))} costCenters={costCenters.map(item => ({ id: item.id, code: item.code, name: item.name }))} role={role} /></div></div>;
}
