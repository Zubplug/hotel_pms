...
export function ExpenseWorkspace({ propertyId, expenses, categories, costCenters, role }: { propertyId: string; expenses: Expense[]; categories: Option[]; costCenters: Option[]; role: string }) {
  ...
  return <>
    <div className="mb-6 flex flex-col gap-4 rounded-2xl border border-indigo-100 bg-indigo-50/60 p-5 sm:flex-row sm:items-center sm:justify-between">
      <div className="flex items-center gap-3"><div className="flex h-11 w-11 items-center justify-center rounded-xl bg-indigo-100 text-indigo-700"><WalletCards className="h-5 w-5" /></div><div><p className="text-sm font-semibold text-slate-900">Controlled cash expenses</p><p className="mt-1 text-xs text-slate-600">Every payout is approved, documented, and paid from the General Cashier Safe.</p></div></div>
    ...
    <div className="mb-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
      {[
        { label: 'Pending approval', value: pendingExpenses.length, detail: `₦${pendingAmount.toLocaleString(undefined, { minimumFractionDigits: 2 })} awaiting decision`, icon: Clock3, tone: 'bg-amber-50 text-amber-700' },
        { label: 'Approved to pay', value: approvedExpenses.length, detail: 'Ready for cashier payment', icon: ShieldCheck, tone: 'bg-blue-50 text-blue-700' },
        { label: 'Paid expenses', value: paidExpenses.length, detail: `₦${paidAmount.toLocaleString(undefined, { minimumFractionDigits: 2 })} disbursed`, icon: CircleDollarSign, tone: 'bg-emerald-50 text-emerald-700' },
        { label: 'Total register', value: expenses.length, detail: 'Controlled expense records', icon: ListChecks, tone: 'bg-indigo-50 text-indigo-700' },
