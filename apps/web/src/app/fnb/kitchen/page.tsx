    ...
    <div className={`flex flex-col overflow-hidden rounded-2xl border ${borderColor} bg-white shadow-xl transition-all`}>
      ...
      <div className="grid grid-cols-2 gap-1 border-t border-slate-200 p-1 bg-slate-50">
        ...
        {batch.status === 'PREPARING' && (
          <>
            <button onClick={() => onTransition(batch.id, 'READY')} className="flex h-10 items-center justify-center gap-2 rounded-xl bg-emerald-500/20 text-sm font-bold text-emerald-300 transition-colors hover:bg-emerald-500/30">
        ...
        {batch.status === 'RECALLED' && (
          <button onClick={() => onTransition(batch.id, 'PENDING')} className="col-span-2 flex h-10 items-center justify-center gap-2 rounded-xl bg-amber-500/20 text-sm font-bold text-amber-300 transition-colors hover:bg-amber-500/30">
