...
export default function NightAuditDashboard() {
  ...
  const checks = useMemo(() => [
    ...
    ...(data?.financial?.pendingCheckInBypasses || []).map((item: any) => ({ label: `Check-In Bypass · ${item.reservation?.confirmationNumber || 'Unknown'}`, tone: 'blocker' })),
  ], [data]);
...
{step === 2 && data?.financial && (
  <div className="space-y-6 border-t pt-4">
    ...
    {data.financial.pendingCheckInBypasses?.length > 0 && <div>
      <div className="mb-3">
        <h4 className="font-semibold text-sm text-rose-700">Check-In Deposit Bypasses (Blocker)</h4>
