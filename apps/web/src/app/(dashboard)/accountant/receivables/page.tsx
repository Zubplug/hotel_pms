    ...
    <div className="min-h-screen bg-slate-950 p-6 font-sans text-slate-50 selection:bg-emerald-500/30 md:p-8">
      <div className="mx-auto max-w-7xl space-y-8">
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
          {aging.map(item => (
            <div key={item.key} className="relative overflow-hidden rounded-xl border border-white/10 bg-white/5 p-5 backdrop-blur-sm">
              ...
              <div className="flex items-end justify-between gap-3"><h3 className="text-2xl font-bold text-rose-500">{formatCurrency(item.amount, currency)}</h3><span className="rounded-full border border-white/10 bg-white/5 px-2 py-1 text-xs text-slate-300">{item.count} account{item.count === 1 ? '' : 's'}</span></div>
        ...
        <div className="flex flex-col justify-between gap-4 rounded-xl border border-white/10 bg-white/5 p-6 backdrop-blur-sm md:flex-row md:items-center">
          <div><p className="text-sm font-medium text-slate-400">Total outstanding receivables</p><h2 className="mt-1 text-4xl font-bold text-rose-500">{formatCurrency(totalOutstanding, currency)}</h2><p className="mt-2 text-sm text-slate-500">{accountRows.length} open account{accountRows.length === 1 ? '' : 's'} · {formatCurrency(overdueAmount, currency)} aged over 30 days</p></div>
