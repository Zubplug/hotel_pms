...
export default async function CashBankPage() {
  ...
  const cashDropPosSessions = posSessions.filter(shift => ['OPEN', 'SUBMITTED', 'UNDER_REVIEW', 'RETURNED', 'HANDOVER_PENDING'].includes(shift.controlStatus));
  const cashDropFrontdeskSessions = frontdeskSessions.filter(shift => ['OPEN', 'CLOSING', 'SUBMITTED', 'UNDER_REVIEW', 'RETURNED', 'HANDOVER_PENDING'].includes(shift.status));
      ...
      <div className="mx-auto max-w-[1540px] space-y-6">
        <header className="flex flex-col justify-between gap-5 lg:flex-row lg:items-end">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {(cashDropPosSessions.length > 0 || cashDropFrontdeskSessions.length > 0) && <RecordCashDropModal posSessions={cashDropPosSessions} frontdeskSessions={cashDropFrontdeskSessions} />}
            <Link href="/accountant/cash-bank/deposits" className="inline-flex items-center gap-2 rounded-xl border border-white/10 bg-white/[.06] px-3 py-2 text-xs font-semibold text-slate-200 hover:bg-white/[.1]"><Landmark className="h-4 w-4" />Deposit control</Link>
        ...
        <section className="flex flex-col justify-between gap-4 rounded-2xl border border-cyan-400/15 bg-cyan-400/[.055] px-5 py-4 sm:flex-row sm:items-center">
          ...
          <Link href="/accountant/cash-bank/deposits" className="inline-flex items-center gap-2 text-xs font-semibold text-cyan-300 hover:text-cyan-200">Review deposit register <ArrowRight className="h-3.5 w-3.5" /></Link>
        </section>
        ...
        <section className="grid gap-5 xl:grid-cols-[1.25fr_.75fr]">
          ...
          <div className="rounded-2xl border border-white/10 bg-[#101b2f] p-5 sm:p-6"><div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[.16em] text-amber-300"><AlertTriangle className="h-4 w-4" />Decision queue</div><h2 className="mt-1 text-lg font-semibold text-white">What needs attention</h2><div className="mt-5 space-y-2">{[
            { label: 'Deposit exceptions', count: exceptions.length, href: '/accountant/cash-bank/deposits', tone: exceptions.length ? 'text-rose-300' : 'text-emerald-300' },
            { label: 'Cash expenses awaiting action', count: pendingExpenses.length, href: '/accountant/cash-bank/expenses', tone: pendingExpenses.length ? 'text-amber-300' : 'text-emerald-300' },
            { label: 'Shifts not fully controlled', count: openShifts.length + openFrontdesk.length, href: '/accountant/cash-bank/handovers', tone: openShifts.length + openFrontdesk.length ? 'text-amber-300' : 'text-emerald-300' },
