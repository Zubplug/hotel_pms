...
export default async function CityLedgerPage() {
  ...
  const totalCredit = accounts.reduce((total, account) => total + Math.abs(Math.min(0, Number(account.balance))), 0);
  const accountsWithBalance = accounts.filter(account => Math.abs(Number(account.balance)) > 0);
