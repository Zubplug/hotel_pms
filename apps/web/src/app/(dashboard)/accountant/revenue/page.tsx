...
type RevenueReport = {
  ...
  businessDate: string;
  snapshot: { today: number; mtd: number; ytd: number; priorYear: number; grossToday: number; discountsToday: number; transactionCountToday: number; variance: number | null; isUp: boolean };
...
export default function RevenueAccountingPage() {
  ...
  const downloadReport = () => {
    ...
    const rows = [['Department', 'Today net', 'Gross charges', 'Discounts', 'MTD', 'YTD', 'Prior year', 'Variance'], ...departments.map(item => [item.department, item.today, item.gross, item.discounts, item.mtd, item.ytd, item.priorYear, item.variance === null ? 'N/A' : `${item.variance}%`]), ['Total', snapshot.today, snapshot.grossToday, snapshot.discountsToday, snapshot.mtd, snapshot.ytd, snapshot.priorYear, snapshot.variance === null ? 'N/A' : `${snapshot.variance}%`]];
    const csv = rows.map(row => row.map(value => `"${String(value).replaceAll('"', '""')}"`).join(',')).join('\n');
    const url = URL.createObjectURL(new Blob([csv], { type: 'text/csv;charset=utf-8' }));
    const anchor = document.createElement('a'); anchor.href = url; anchor.download = `revenue-audit-${businessDate}.csv`; anchor.click(); URL.revokeObjectURL(url);
  };
  ...
  if (reportQuery.isError || !reportQuery.data) return <div className="flex min-h-full flex-col items-center justify-center gap-4 bg-[#09111f] p-8 text-center text-slate-300"><FileWarning className="h-10 w-10 text-rose-300" /><p>Revenue data could not be loaded.</p><button onClick={() => reportQuery.refetch()} className="inline-flex items-center gap-2 rounded-xl border border-white/10 px-4 py-2 text-sm hover:bg-white/10"><RefreshCw className="h-4 w-4" />Retry</button></div>;
  ...
  return <div className="min-h-full bg-[#09111f] px-4 py-6 text-slate-200 sm:px-6 lg:px-8 lg:py-8"><div className="mx-auto max-w-[1480px] space-y-6">
    ...
    <div className="grid gap-6 xl:grid-cols-[minmax(0,1.35fr)_minmax(320px,.65fr)]"><Panel><div className="flex items-center justify-between border-b border-white/[0.07] px-5 py-4"><div><div className="flex items-center gap-2 text-sm font-semibold text-white"><BarChart3 className="h-4 w-4 text-violet-300" /> Department performance</div><p className="mt-1 text-xs text-slate-500">A clean audit view of production, mix, volume, and same-day prior-year variance</p></div><button className="rounded-lg p-1.5 text-slate-500 hover:bg-white/5 hover:text-white" title="Filter departments"><Filter className="h-4 w-4" /></button></div><div className="overflow-x-auto"><table className="w-full min-w-[800px] text-left"><thead><tr className="border-b border-white/[0.07] text-[10px] uppercase tracking-[.14em] text-slate-500"><th className="px-5 py-3 font-semibold">Department</th><th className="px-5 py-3 font-semibold text-right">Net today</th><th className="px-5 py-3 font-semibold text-right">Gross / discounts</th><th className="px-5 py-3 font-semibold text-right">MTD</th><th className="px-5 py-3 font-semibold text-right">YoY</th></tr></thead><tbody className="divide-y divide-white/[0.06]">{departments.map(item => <tr key={item.id} className="group transition hover:bg-white/[0.03]"><td className="px-5 py-4"><div className="font-medium text-slate-200">{item.department}</div><div className="mt-1 text-[11px] text-slate-500">{item.count.toLocaleString()} charge transactions</div></td><td className="px-5 py-4 text-right font-semibold text-white">{money(item.today, report.property.currency, true)}</td><td className="px-5 py-4 text-right text-xs"><div className="text-slate-300">{money(item.gross, report.property.currency, true)}</div><div className="mt-1 text-amber-300/80">−{money(item.discounts, report.property.currency, true)}</div></td><td className="px-5 py-4 text-right text-slate-300">{money(item.mtd, report.property.currency, true)}</td><td className="px-5 py-4 text-right">{item.variance === null ? <span className="text-xs text-slate-600">No baseline</span> : <span className={`inline-flex items-center gap-1 text-xs font-semibold ${item.isUp ? 'text-emerald-300' : 'text-rose-300'}`}>{item.isUp ? <ArrowUpRight className="h-3.5 w-3.5" /> : <ArrowDownRight className="h-3.5 w-3.5" />}{pct(item.variance)}</span>}</td></tr>)}</tbody></table></div></Panel>
