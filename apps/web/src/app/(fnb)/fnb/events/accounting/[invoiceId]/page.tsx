...
import { finalizeEventInvoice } from '@/lib/events/accounting-actions';
...
export default async function EventInvoicePage({ params }: { params: Promise<{ invoiceId: string }> }) {
  ...
  const finalizeAction = async () => {
    'use server';
    await finalizeEventInvoice(invoice.id);
  };

  return <div className="space-y-6 p-6"><Button variant="link" asChild className="-ml-4 px-4"><Link href="/fnb/events/accounting">← Back to accounting</Link></Button><Card><CardHeader><CardTitle>Invoice {invoice.id.slice(0, 8).toUpperCase()}</CardTitle><p className="text-sm text-muted-foreground">{invoice.event?.name || 'Event invoice'} · {invoice.status}</p></CardHeader><CardContent><div className="space-y-3">{invoice.items.map(item => <div key={item.id} className="flex justify-between border-b pb-2 text-sm"><span>{item.description} × {item.quantity}</span><span>{Number(item.totalPrice).toLocaleString()} {invoice.currency}</span></div>)}<div className="flex justify-between font-semibold"><span>Total</span><span>{Number(invoice.totalAmount).toLocaleString()} {invoice.currency}</span></div><div className="flex justify-between text-sm text-muted-foreground"><span>Paid</span><span>{Number(invoice.paidAmount).toLocaleString()} {invoice.currency}</span></div><div className="flex flex-wrap gap-2 pt-4">{['DRAFT', 'UNPAID'].includes(invoice.status) && <form action={finalizeAction}><Button type="submit">Issue invoice</Button></form>}{invoice.folioId && !['DRAFT', 'VOID'].includes(invoice.status) && <Button variant="outline" asChild><Link href={`/frontdesk/folios/${invoice.folioId}?eventInvoiceId=${invoice.id}`}>Open individual folio / receive payment</Link></Button>}</div></div></CardContent></Card></div>;
}
