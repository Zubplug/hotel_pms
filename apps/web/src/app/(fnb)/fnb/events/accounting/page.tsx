...
export default async function EventAccountingPage() {
  ...
  const invoiceWhere = { event: { propertyId } };
  const [invoices, allInvoices, totals, workflowGroups] = await Promise.all([
    prisma.eventInvoice.findMany({ where: invoiceWhere, orderBy: { createdAt: 'desc' }, include: { event: true }, take: 50 }),
  ...
  return <div className="fnb-dark-surface min-h-full bg-[#fbf8f6] text-[#24130d]">
    ...
    <main className="mx-auto max-w-[1600px] space-y-6 px-4 py-6 sm:px-6 lg:px-8">
      ...
      <section className="overflow-hidden rounded-2xl border border-[#eadfd8] bg-white shadow-[0_8px_24px_rgba(65,32,19,0.045)]"><div className="flex flex-col justify-between gap-3 border-b border-[#eadfd8] px-5 py-5 sm:flex-row sm:items-center"><div><h2 className="text-base font-bold">Invoice control ledger</h2><p className="mt-1 text-xs text-[#947d72]">The latest 50 event invoices with financial status and approval workflow kept separate.</p></div><div className="flex items-center gap-2 text-xs text-[#947d72]"><Clock3 className="h-4 w-4" /> Live property ledger</div></div><div className="overflow-x-auto"><table className="w-full min-w-[940px] text-left text-sm"><thead className="bg-[#fffaf7] text-[10px] uppercase tracking-[0.12em] text-[#947d72]"><tr><th className="px-5 py-3">Event / invoice</th><th className="px-5 py-3">Created</th><th className="px-5 py-3">Financial status</th><th className="px-5 py-3">Approval workflow</th><th className="px-5 py-3 text-right">Balance</th><th className="px-5 py-3 text-right">Action</th></tr></thead><tbody className="divide-y divide-[#f0e6e0]">{invoices.length === 0 ? <tr><td colSpan={6} className="px-5 py-14 text-center text-sm text-[#947d72]">No event invoices have been generated yet.</td></tr> : invoices.map((invoice) => { const balance = Math.max(Number(invoice.totalAmount) - Number(invoice.paidAmount), 0); return <tr key={invoice.id} className="transition hover:bg-[#fffaf7]"><td className="px-5 py-4"><p className="font-semibold text-[#24130d]">{invoice.event?.name || 'Unlinked event'}</p><p className="mt-1 text-[10px] uppercase tracking-wider text-[#947d72]">INV-{invoice.id.slice(0, 8).toUpperCase()} · {invoice.event?.contactName || 'No contact'}</p></td><td className="px-5 py-4 text-xs text-[#6f5d53]">{invoice.createdAt.toLocaleDateString('en-NG', { day: '2-digit', month: 'short', year: 'numeric' })}</td><td className="px-5 py-4"><span className={`rounded-full px-2.5 py-1 text-[10px] font-bold uppercase ring-1 ${tone(invoice.status, 'financial')}`}>{label(invoice.status)}</span></td><td className="px-5 py-4"><span className={`rounded-full px-2.5 py-1 text-[10px] font-bold uppercase ring-1 ${tone(invoice.workflowStatus, 'workflow')}`}>{label(invoice.workflowStatus)}</span></td><td className="px-5 py-4 text-right"><p className="font-bold">{money(balance, invoice.currency)}</p><p className="mt-1 text-[10px] text-[#947d72]">of {money(Number(invoice.totalAmount), invoice.currency)}</p></td><td className="px-5 py-4 text-right"><ViewInvoiceButton invoiceId={invoice.id} /></td></tr>; })}</tbody></table></div><div className="flex items-center justify-between border-t border-[#eadfd8] bg-[#fffaf7] px-5 py-3 text-xs text-[#947d72]"><span>Showing {invoices.length} of {allInvoices.length} invoice{allInvoices.length === 1 ? '' : 's'}</span><Link href="/fnb/events/bookings" className="font-bold text-orange-700">Open event register <ChevronRight className="inline h-3 w-3" /></Link></div></section>
    </main>
  </div>;
}
