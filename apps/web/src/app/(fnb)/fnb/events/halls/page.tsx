...
import { CalendarDays, CheckCircle2, LayoutGrid, Plus, Ruler, Sparkles, Tags, Users } from 'lucide-react';
...
export default async function HallsPage() {
  ...
  return <div className="min-h-full bg-[#fbf8f6] text-[#24130d]">
    <header className="border-b border-[#3d2318] bg-[#24130d] text-white"><div className="mx-auto max-w-[1600px] px-4 py-7 sm:px-6 lg:px-8"><div className="flex flex-col justify-between gap-6 xl:flex-row xl:items-end"><div><div className="mb-3 flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.2em] text-orange-300"><Sparkles className="h-4 w-4" /> F&B Hall & events</div><h1 className="text-3xl font-bold tracking-tight sm:text-4xl">Halls & spaces</h1><p className="mt-2 max-w-2xl text-sm text-orange-100/75">Maintain the function-space inventory your sales and banquet teams can confidently sell.</p></div><div className="flex flex-wrap gap-2"><EventScheduleDialog /><HallForm /></div></div></div></header>
    <main className="mx-auto max-w-[1600px] space-y-6 px-4 py-6 sm:px-6 lg:px-8">
      ...
      <section className="rounded-2xl border border-orange-200 bg-gradient-to-r from-[#fff3e8] to-white p-5 shadow-[0_8px_24px_rgba(65,32,19,0.045)]"><div className="flex items-center gap-2 text-sm font-bold text-[#7c2d12]"><CheckCircle2 className="h-4 w-4" /> Booking-safe catalog</div><p className="mt-1 text-xs text-[#947d72]">Capacity and schedule remain the source of truth for the booking flow. Changes here apply to new bookings immediately.</p></section>
