...
export async function POST(req: NextRequest, { params }: { params: Promise<{ shiftId: string }> }) {
  try {
    ...
    if (!((await requireOrganizationContext(actorUserId)).propertyIds).includes(propertyId)) {
      return errorResponse('FORBIDDEN', 'No access to this property', 403);
    }
    let updated;
    if (decision === 'APPROVED') {
      updated = await ShiftControlService.approveShift(await requireOrganizationContext(actorUserId), type, shiftId);
