...
export async function POST(req: NextRequest) {
  ...
  const pending = await prisma.approvalRequest.findFirst({ where: { propertyId: product.propertyId, type: 'POS_MODIFIER_CREATE', status: 'PENDING', details: { path: ['productId'], equals: productId } } });
  if (pending) return errorResponse('CONFLICT', 'This product already has a pending modifier request', 409);
  const approval = await prisma.approvalRequest.create({ data: { propertyId: product.propertyId, type: 'POS_MODIFIER_CREATE', status: 'PENDING', requestedBy: user.id, amount: price, currency: 'NGN', reason: String(body.reason || `New modifier requested: ${name}`), details: { stage: 'ACCOUNTANT_REVIEW', productId, productName: product.name, name, price, stockItemId: stockItem?.id || null, quantity, unitOfMeasure: body.unitOfMeasure || stockItem?.baseUnit || null } } });
