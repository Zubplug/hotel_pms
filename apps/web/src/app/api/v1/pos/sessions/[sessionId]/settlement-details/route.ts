...
export async function GET(_request: NextRequest, { params }: { params: Promise<{ sessionId: string }> }) {
  try {
    ...
    return NextResponse.json({ data: {
      ...
      otherSales: posSession.payments.filter(payment => !['CASH', 'CARD', 'CARD_OFFLINE', 'POS', 'BANK_TRANSFER', 'ROOM_CHARGE', 'COMPLIMENTARY'].includes(payment.method)).reduce((sum, payment) => sum + n(payment.amount), 0),
      complimentarySales: totalBy('COMPLIMENTARY'),
      totalSales: posSession.payments.filter(payment => payment.method !== 'COMPLIMENTARY').reduce((sum, payment) => sum + n(payment.amount), 0),
