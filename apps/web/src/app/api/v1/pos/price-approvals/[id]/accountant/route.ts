...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!approval || !['POS_PRICE_CHANGE', 'POS_MENU_CREATE', 'POS_MODIFIER_CREATE'].includes(approval.type)) return errorResponse('NOT_FOUND', 'POS approval request not found', 404);
