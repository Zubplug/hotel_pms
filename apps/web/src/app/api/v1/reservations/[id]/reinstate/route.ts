...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const reinstated = await prisma.$transaction(async tx => {
      ...
      const propertyBusinessDateStr = (reservation.property.businessDate ?? new Date()).toISOString().split('T')[0];
      for (const roomAssignment of reservation.reservationRooms) {
        if (!roomAssignment.roomId) continue;
        const checkInStr = roomAssignment.checkIn.toISOString().split('T')[0];
