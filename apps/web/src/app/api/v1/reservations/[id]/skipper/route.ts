...
export async function POST(
  ...
  try {
    ...
    if (!reason || typeof reason !== 'string' || reason.trim() === '') {
      return errorResponse('INVALID_REQUEST', 'A reason is required to transfer a balance to the City Ledger', 400);
    }
    ...
    if (reservation.status !== 'CHECKED_IN') {
      return errorResponse('INVALID_STATE', `Cannot process skipper checkout for status ${reservation.status}`, 409);
    }
    ...
    const txResult = await prisma.$transaction(async (tx: any) => {
      ...
      // 2. Find or Create City Ledger Account for Skippers
      let skipperAccount = await tx.cityLedgerAccount.findFirst({
        ...
        if (folioBalance > 0) {
          ...
          await tx.folioItem.create({
            data: {
              ...
              type: 'PAYMENT',
              source: 'CITY_LEDGER',
              description: `City Ledger Transfer - Skipper (Ref: ${clEntry.id})`,
              quantity: 1,
        ...
        await tx.auditLog.create({
          data: {
            ...
            newValue: { 
              ...
              transferredAmount: totalBalance,
              reason
            },
    ...
    try {
      ...
      console.error('[Skipper-Check-Out] Revocation dispatch failed (non-blocking):', hwErr);
    }

    return successResponse({
      message: 'Skipper transferred to City Ledger and check-out complete.',
      revokedCredentials: revokedCount,
    });

  } catch (err: unknown) {
    const message = err instanceof Error ? err.message : String(err);
    console.error('[Skipper POST]', err);

    if (message === 'INVALID_BALANCE') {
      return errorResponse('INVALID_BALANCE', 'A skipper check-out requires a positive outstanding balance.', 400);
    }
