...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const reservation = await prisma.reservation.findUnique({ where: { id }, select: { propertyId: true, status: true } });
    ...
    const updated = await prisma.reservation.update({ where: { id }, data: { lateArrivalExpected: true, lateArrivalNotes: String(body.notes || '').trim() || null, lateArrivalAt: new Date(), lateArrivalBy: session.user.id } });
