...
async function getContext(permission?: 'inventory.manage') {
  ...
  if (!user.propertyId || (permission && !hasInventoryPermission(user.role, permission, user.isSuperAdmin))) return { response: NextResponse.json({ error: 'Forbidden' }, { status: 403 }) };
  return { user };
}
...
export async function GET() {
  ...
  const recipes = await prisma.recipe.findMany({ where: { propertyId: context.user.propertyId, isActive: true }, include: { posProduct: { select: { id: true, name: true, price: true, inventoryMode: true } }, versions: { where: { isActive: true }, take: 1, include: { ingredients: { include: { stockItem: { select: { id: true, name: true, baseUnit: true, quantityOnHand: true, costPrice: true, stockUnits: true } } } } } } }, orderBy: { updatedAt: 'desc' } });
  const [products, stockItems] = await Promise.all([
    prisma.posProduct.findMany({ where: { propertyId: context.user.propertyId, isActive: true }, select: { id: true, name: true, price: true, inventoryMode: true }, orderBy: { name: 'asc' } }),
    prisma.stockItem.findMany({ where: { propertyId: context.user.propertyId, isActive: true }, select: { id: true, name: true, baseUnit: true, costPrice: true, quantityOnHand: true, stockUnits: true }, orderBy: { name: 'asc' } }),
  ]);
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const product = await prisma.posProduct.findFirst({ where: { id: body.posProductId, propertyId: context.user.propertyId, isActive: true } });
    if (!product) return NextResponse.json({ error: 'POS product not found' }, { status: 404 });
    const ingredients = Array.isArray(body.ingredients) ? body.ingredients : [];
    if (!ingredients.length) return NextResponse.json({ error: 'At least one ingredient is required' }, { status: 400 });
    const stock = await prisma.stockItem.findMany({ where: { propertyId: context.user.propertyId, id: { in: ingredients.map((item: any) => item.stockItemId) }, isActive: true } });
    ...
    const recipe = await prisma.$transaction(async (tx) => {
      ...
      const saved = existing || await tx.recipe.create({ data: { propertyId: context.user.propertyId, posProductId: product.id, targetMargin: Number(body.targetMargin ?? 70) } });
