...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const expected = current.cashMovements.reduce((total, movement) => {
      const amount = Number(movement.amount);
      return ['OPENING_FLOAT', 'PAYMENT', 'CASH_TRANSFER_IN'].includes(movement.type) ? total + amount : ['REFUND', 'PAID_OUT', 'CASH_DROP', 'CASH_TRANSFER_OUT'].includes(movement.type) ? total - amount : total + (movement.type === 'ADJUSTMENT' ? amount : 0);
    }, 0);
    const declared = declaredCash === undefined ? 0 : Number(declaredCash);
    const variance = declared - expected;
    const closed = await prisma.$transaction(async tx => {
      const updated = await tx.frontdeskSession.update({ where: { id }, data: { status: 'CLOSED', closingAt: new Date(), closedAt: new Date(), declaredCash: declared, systemExpectedCash: expected, variance } });
