...
export async function POST(req: NextRequest, { params }: { params: Promise<{ entryId: string }> }) {
  try {
    ...
    const result = await prisma.$transaction(async tx => {
      ...
      await tx.approvalRequest.create({ data: { propertyId: entry.propertyId, type: 'REFUND', status: 'PENDING', requestedBy: session.user.id, amount, currency: entry.currency, reason, details: { refundRequestId: request.id, cityLedgerEntryId: entry.id, category: request.category, requestedAmount: amount, requestedMethod }, expiresAt: request.expiresAt } });
      return request;
    });
