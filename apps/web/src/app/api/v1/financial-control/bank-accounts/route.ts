...
async function adminContext() {
  ...
  return { actor, role, ctx: await requireOrganizationContext(actor.user.id) };
}
export async function GET(request: Request) {
  const actor = await auth();
  if (!actor?.user) return NextResponse.json({ error: 'Unauthorized' }, { status: 401 });
  const propertyId = new URL(request.url).searchParams.get('propertyId');
  if (!propertyId || !((await requireOrganizationContext(actor.user.id)).propertyIds).includes(propertyId)) return NextResponse.json({ error: 'Forbidden' }, { status: 403 });
  ...
  const accounts = existing.length > 0 ? existing : [await ensureBankAccountForClient(await requireOrganizationContext(actor.user.id), prisma, propertyId)];
...
export async function POST(request: Request) {
  try {
    ...
    if (!propertyId || !context.ctx.propertyIds.includes(propertyId)) return NextResponse.json({ error: 'Invalid property' }, { status: 403 });
