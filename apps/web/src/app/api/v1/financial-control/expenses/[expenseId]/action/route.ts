...
export async function POST(request: NextRequest, context: { params: Promise<{ expenseId: string }> }) {
  try {
    ...
    if (!expense || !(await getUserPropertyIds(session.user.id)).includes(expense.propertyId)) return NextResponse.json({ error: 'Expense not found' }, { status: 404 });
    ...
    if (action === 'approve') return NextResponse.json({ data: await CashExpenseService.approve(expenseId, staff.id, body.notes) });
    if (action === 'reject') return NextResponse.json({ data: await CashExpenseService.reject(expenseId, staff.id, String(body.reason || '')) });
