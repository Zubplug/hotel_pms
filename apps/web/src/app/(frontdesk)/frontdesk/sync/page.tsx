...
export default function SyncCenterPage() {
  ...
  const isAuthorized = ['ADMIN', 'MANAGER', 'SYSTEM_ADMIN', 'RECEPTIONIST', 'FRONT_DESK'].includes(role || '');
  ...
  const events = useMemo(() => [...(data?.frontDesk || []), ...(data?.pos || [])].sort((a, b) => {
    ...
    <div className="min-h-screen bg-slate-50 p-6 md:p-8 pb-24">
      <div className="max-w-6xl mx-auto space-y-6">
        <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
          <div>
            <Button variant="outline" onClick={() => router.push('/frontdesk')} className="rounded-full mb-5"><ArrowLeft className="mr-2 h-4 w-4" />Back</Button>
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
          {[
            ['Pending', pending.length, 'text-amber-600'],
            ['Conflicts', conflicts.length, 'text-red-600'],
            ['Dead Letters', deadLetters.length, 'text-rose-700'],
            ['Synced', synced.length, 'text-emerald-600'],
            ['Total', events.length, 'text-slate-900'],
          ].map(([label, count, color]) => <div key={String(label)} className="bg-white rounded-2xl border border-slate-200 p-4"><p className="text-xs font-bold uppercase text-slate-400">{label}</p><p className={`text-2xl font-black ${color}`}>{count}</p></div>)}
        </div>

        {events.length === 0 ? <div className="bg-white rounded-2xl border border-dashed border-slate-300 p-12 text-center text-slate-500"><CheckCircle2 className="mx-auto mb-3 text-emerald-500" /><p>No local sync events found.</p></div> : <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
          <div className="p-5 border-b border-slate-200 flex items-center justify-between"><h2 className="font-bold text-slate-900">All Local Push Events</h2><span className="text-xs text-slate-500">Auto-refreshes every 5 seconds</span></div>
          <div className="divide-y divide-slate-100">
            {events.map(event => <div key={`${event.source}-${event.id}`} className="p-5 flex flex-col md:flex-row md:items-center gap-4 justify-between">
              <div className="min-w-0">
                <div className="flex flex-wrap items-center gap-2 mb-1"><span className={`text-xs font-black px-2 py-1 rounded-full ${event.source === 'POS' ? 'bg-indigo-100 text-indigo-700' : 'bg-blue-100 text-blue-700'}`}>{event.source}</span><span className="font-bold text-slate-800">{event.type}</span><span className="text-xs text-slate-500">{event.aggregate}</span></div>
                <p className="text-xs font-mono text-slate-400 truncate">ID: {event.id}</p>
                {event.error && <p className="mt-2 text-sm font-medium text-red-700 bg-red-50 rounded-lg px-3 py-2 break-words">{event.error}</p>}
              </div>
              <div className="flex items-center gap-4 md:justify-end shrink-0"><div className="text-right text-xs text-slate-500"><p className="font-bold">{event.status}</p><p>Attempts: {event.attempts}</p>{event.createdAt && <p>{new Date(event.createdAt).toLocaleString()}</p>}</div>{event.status === 'SYNCED' ? <CheckCircle2 className="text-emerald-500" /> : event.status === 'CONFLICT' ? <AlertTriangle className="text-red-500" /> : event.status === 'FAILED' || deadLetters.some(item => item.id === event.id) ? <XCircle className="text-rose-500" /> : <Clock className="text-amber-500" />}</div>
            </div>)}
          </div>
        </div>}
      </div>
