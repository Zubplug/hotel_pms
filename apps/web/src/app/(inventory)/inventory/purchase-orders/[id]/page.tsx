            ...
            <tbody className="divide-y divide-slate-800">
              {po.items.map((item: any) => (
                <tr key={item.id} className="hover:bg-slate-800/50 transition-colors">
                  ...
                  <td className="px-6 py-4 text-slate-300 text-right">{Number(item.unitPrice || 0).toFixed(2)}</td>
                  <td className="px-6 py-4 text-slate-300 text-right">{Number(item.totalPrice || 0).toFixed(2)}</td>
                  <td className="px-6 py-4 text-right">
                    <span className="inline-flex items-center justify-center px-2 py-0.5 rounded text-xs font-medium bg-slate-800 text-slate-300">
