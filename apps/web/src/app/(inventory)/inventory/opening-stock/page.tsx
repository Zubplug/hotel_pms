...
type Unit = { unit: string; unitsInBase: number | string };
...
export default function OpeningStockPage() {
  ...
  const unitOptions = item ? [{ unit: item.baseUnit, unitsInBase: 1 }, ...(item.stockUnits || []).filter((unit) => unit.unit !== item.baseUnit)] : [];
  const selectedConversion = Number(unitOptions.find((unit) => unit.unit === inputUnit)?.unitsInBase || 0);
  ...
  async function submit(event: React.FormEvent) {
    ...
    try {
      const res = await fetch('/api/v1/inventory/opening-stock', {
        ...
        body: JSON.stringify({ warehouseId, stockItemId, quantity: nextQty, inputUnit, unitCost: nextCost, notes, operationId: crypto.randomUUID() }),
      });
    ...
    <div className="mx-auto max-w-2xl space-y-5 px-6 py-7">
      ...
      <form onSubmit={submit} className="space-y-5 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        {loading ? <div className="flex justify-center p-8"><Loader2 className="animate-spin text-emerald-600" /></div> : <>
          <label className="block text-sm font-semibold text-slate-700">Main warehouse<select value={warehouseId} onChange={(e) => { setWarehouseId(e.target.value); setStockItemId(''); setInputUnit(''); }} className="mt-1.5 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5" required>{warehouses.map((store) => <option key={store.id} value={store.id}>{store.name}</option>)}</select></label>
          <label className="block text-sm font-semibold text-slate-700">Stock item<select value={stockItemId} onChange={(e) => { setStockItemId(e.target.value); const selected = items.find((stock) => stock.id === e.target.value); setInputUnit(selected?.stockUnits?.find((unit) => (unit as any).isPurchaseUnit)?.unit || selected?.baseUnit || ''); }} className="mt-1.5 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5" required><option value="">Choose an item…</option>{items.map((stock) => <option key={stock.id} value={stock.id}>{stock.name}{stock.sku ? ` (${stock.sku})` : ''} · Current {Number(stock.quantityOnHand).toFixed(2)} {stock.baseUnit}</option>)}</select></label>
          <div className="grid gap-4 sm:grid-cols-3"><label className="block text-sm font-semibold text-slate-700">Purchase quantity<input type="number" min="0.0001" step="0.0001" value={quantity} onChange={(e) => setQuantity(e.target.value)} className="mt-1.5 w-full rounded-xl border border-slate-200 px-3 py-2.5" required /></label><label className="block text-sm font-semibold text-slate-700">Purchase unit<select value={inputUnit} onChange={(e) => setInputUnit(e.target.value)} className="mt-1.5 w-full rounded-xl border border-slate-200 px-3 py-2.5" required disabled={!item}><option value="">Choose unit…</option>{unitOptions.map((unit) => <option key={unit.unit} value={unit.unit}>{unit.unit}{Number(unit.unitsInBase) > 1 ? ` · ${unit.unitsInBase} ${item?.baseUnit}` : ''}</option>)}</select></label><label className="block text-sm font-semibold text-slate-700">Cost per purchase unit (₦)<input type="number" min="0" step="0.01" value={unitCost} onChange={(e) => setUnitCost(e.target.value)} className="mt-1.5 w-full rounded-xl border border-slate-200 px-3 py-2.5" required /></label></div>
