...
export default async function CostControlDashboardPage() {
  ...
  const propertyIds = session?.user ? [...(await requireOrganizationContext(session.user.id)).propertyIds] : [];
  ...
  const theoreticalCost = orders.reduce((sum, order) => sum + order.items.reduce((itemTotal: number, item: any) => {
    if (item.product?.inventoryMode !== 'STOCK') return itemTotal;
    const ingredients = item.product.recipe?.versions[0]?.ingredients || [];
    return itemTotal + ingredients.reduce((ingredientTotal: number, ingredient: any) => ingredientTotal + baseQuantity(ingredient.quantity, ingredient.unitOfMeasure, ingredient.stockItem.baseUnit, ingredient.stockItem.stockUnits) * number(ingredient.stockItem.costPrice) * number(item.quantity), 0);
  }, 0), 0);
  ...
  const kpis = [
    ...
    { label: 'Food Cost %', value: `${foodCostPercent.toFixed(1)}%`, detail: revenue ? `${formatCurrency(actualCost, currency)} consumed against sales` : 'No closed POS sales this month', icon: foodCostPercent <= 30 ? TrendingDown : TrendingUp, tone: foodCostPercent <= 30 ? 'emerald' : 'red' as const },
    { label: 'Theoretical Cost (MTD)', value: formatCurrency(theoreticalCost, currency), detail: 'Based on closed POS sales and active recipes', icon: FileText, tone: 'blue' as const },
    { label: 'Actual Cost (MTD)', value: formatCurrency(actualCost, currency), detail: 'Posted stock consumption from POS sales', icon: Activity, tone: 'violet' as const },
    { label: 'Cost Variance', value: formatCurrency(variance, currency), detail: variance <= 0 ? 'At or below theoretical cost' : 'Actual consumption above theoretical', icon: variance <= 0 ? TrendingDown : TrendingUp, tone: variance <= 0 ? 'emerald' : 'red' as const },
  ];
