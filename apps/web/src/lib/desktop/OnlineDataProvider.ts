...
export const OnlineDataProvider: LodgeCoreDataProvider = {
  ...
  frontdesk: {
    ...
    async getSessionSummary(sessionId: string) {
      return apiFetch(`/api/v1/frontdesk/sessions/${encodeURIComponent(sessionId)}/summary`);
    },
  ...
  reservations: {
    ...
    checkIn: async (id, userId, deviceId, options) => {
      return apiFetch(`/api/v1/reservations/${id}/check-in`, {
        method: 'POST',
        body: JSON.stringify({ userId, deviceId, options })
      });
    },
