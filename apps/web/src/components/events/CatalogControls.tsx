...
export function PackageForm({ pkg, products = [] }: { pkg?: { id: string; name: string; description: string | null; basePrice: string; isHallOnly: boolean; items?: PackageLineItem[] }; products?: PackageProduct[] }) {
  ...
  return <Dialog open={open} onOpenChange={setOpen}>
    ...
    <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-[680px]">
      ...
      <form onSubmit={event => { event.preventDefault(); submit(event.currentTarget); }} className="space-y-5"><section className="space-y-4"><div className="text-[10px] font-bold uppercase tracking-[0.16em] text-orange-700">Commercial details</div><div className="grid gap-4 sm:grid-cols-2"><label className="space-y-1.5 text-xs font-semibold text-[#6f5d53] sm:col-span-2">Package name<Input name="name" defaultValue={pkg?.name} placeholder="e.g. Executive Conference Package" required className="mt-1.5" /></label><label className="space-y-1.5 text-xs font-semibold text-[#6f5d53] sm:col-span-2">Sales description<Input name="description" defaultValue={pkg?.description || ''} placeholder="What is included and who is this offer for?" className="mt-1.5" /></label><label className="space-y-1.5 text-xs font-semibold text-[#6f5d53]">Base price<input name="basePrice" defaultValue={pkg?.basePrice} type="number" min="0" step="0.01" placeholder="0" required className="mt-1.5 flex h-8 w-full rounded-lg border border-input bg-background px-2.5 text-sm outline-none focus:ring-2 focus:ring-orange-500/30" /><span className="block text-[10px] font-normal text-[#947d72]">Default price shown in sales and booking.</span></label><label className="flex items-center gap-3 self-end rounded-xl border border-[#eadfd8] bg-[#fff8f2] p-3 text-sm font-medium text-[#3d2318]"><input name="isHallOnly" type="checkbox" defaultChecked={pkg?.isHallOnly} className="h-4 w-4 accent-orange-600" /><span><span className="block">Hall-only offer</span><span className="block text-xs font-normal text-[#947d72]">No catering included.</span></span></label></div></section><section className="space-y-3 border-t border-[#eadfd8] pt-5"><div className="flex items-center justify-between"><div><p className="text-[10px] font-bold uppercase tracking-[0.16em] text-orange-700">Package composition</p><p className="mt-1 text-xs text-[#947d72]">Attach active POS products for kitchen and BEO handoff.</p></div><Button type="button" variant="outline" size="sm" onClick={() => setItems((current) => [...current, { posProductId: '', quantity: 1, priceOverride: '' }])} disabled={!products.length}> <Plus className="mr-1 h-3.5 w-3.5" /> Add item</Button></div>{!products.length ? <p className="rounded-xl border border-dashed border-[#d9c8bd] p-4 text-xs text-[#947d72]">No active POS products are available to attach yet.</p> : items.length === 0 ? <p className="rounded-xl border border-dashed border-[#d9c8bd] p-4 text-xs text-[#947d72]">No package items added. You can create the package with just its commercial details.</p> : <div className="space-y-2">{items.map((item, index) => <div key={`${index}-${item.posProductId}`} className="grid gap-2 rounded-xl border border-[#eadfd8] bg-[#fffaf7] p-3 sm:grid-cols-[1fr_90px_120px_32px]"><select value={item.posProductId} onChange={(event) => setItems((current) => current.map((row, rowIndex) => rowIndex === index ? { ...row, posProductId: event.target.value } : row))} className="h-8 rounded-lg border border-input bg-white px-2 text-xs"><option value="">Select POS product</option>{products.map((product) => <option key={product.id} value={product.id}>{product.name} · ₦{Number(product.price).toLocaleString('en-NG')}</option>)}</select><input aria-label="Quantity" type="number" min="1" value={item.quantity} onChange={(event) => setItems((current) => current.map((row, rowIndex) => rowIndex === index ? { ...row, quantity: Number(event.target.value) } : row))} className="h-8 rounded-lg border border-input bg-white px-2 text-xs" placeholder="Qty" /><input aria-label="Price override" type="number" min="0" step="0.01" value={item.priceOverride} onChange={(event) => setItems((current) => current.map((row, rowIndex) => rowIndex === index ? { ...row, priceOverride: event.target.value } : row))} className="h-8 rounded-lg border border-input bg-white px-2 text-xs" placeholder="Price override" /><Button type="button" variant="ghost" size="icon-sm" onClick={() => setItems((current) => current.filter((_, rowIndex) => rowIndex !== index))}>×</Button></div>)}</div>}</section><DialogFooter><Button type="button" variant="outline" onClick={() => setOpen(false)}>Cancel</Button><SubmitButton editing={Boolean(pkg)} pending={pending} noun="package" /></DialogFooter></form>
    </DialogContent>
  </Dialog>;
}
