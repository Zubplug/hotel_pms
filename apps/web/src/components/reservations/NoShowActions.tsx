...
export function NoShowActions({ reservation, onUpdated }: { reservation: any; onUpdated?: () => void }) {
  ...
  const arrivalOverdue = reservation.status === 'CONFIRMED' && isArrivalOverdue(reservation);
  ...
  if (reservation.status === 'CONFIRMED') return <div className="rounded-lg border border-amber-200 bg-amber-50 p-4 space-y-3"><div className="flex items-center justify-between gap-3"><div className="flex items-center gap-2 font-semibold text-amber-900"><Clock3 className="h-4 w-4" /> Arrival management</div>{arrivalOverdue && <span className="rounded-full bg-red-100 px-2.5 py-1 text-xs font-semibold text-red-800">Arrival date passed</span>}</div>{arrivalOverdue && <p className="text-sm text-red-800">Review this arrival. Record a late arrival or assess no-show; the property policy will be checked before completion.</p>}{!expanded ? <div className="flex flex-wrap gap-2"><Button size="sm" variant="outline" onClick={() => setExpanded(true)}>Record late arrival</Button><Button size="sm" variant="destructive" disabled={busy} onClick={async () => { const data = await request('no-show', {}); if (data) { setAssessment(data.assessment); onUpdated?.(); } }}>{arrivalOverdue ? 'Assess no-show now' : 'Assess no-show'}</Button></div> : <div className="space-y-2"><Textarea value={notes} onChange={event => setNotes(event.target.value)} placeholder="Expected arrival time or guest notes" /><div className="flex gap-2"><Button size="sm" disabled={busy} onClick={async () => { const data = await request('late-arrival', { notes }); if (data) { setExpanded(false); onUpdated?.(); } }}>{busy && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}Save late arrival</Button><Button size="sm" variant="ghost" onClick={() => setExpanded(false)}>Cancel</Button></div></div>}{message && <p className="text-sm text-red-700">{message}</p>}{assessment && <AssessmentCard assessment={assessment} />}</div>;
