...
export function FrontDeskCityLedgerPaymentDialog({ entry, onComplete }: { entry: any; onComplete: () => Promise<any> | void }) {
  ...
  async function submit(event: React.FormEvent) {
    ...
    if (!Number.isFinite(numericAmount) || numericAmount <= 0 || numericAmount > Number(entry.outstandingAmount) + 0.01) {
    ...
    setBusy(true);
    try {
      const result = await provider.cityLedger.settle({ entryId: entry.entryId, accountId: entry.accountId, invoiceId: isCorporate ? undefined : entry.invoiceId, accountType: entry.accountType, amount: numericAmount, method, reference: reference.trim() });
  ...
  return <Dialog open={open} onOpenChange={setOpen}>
    <DialogTrigger render={<Button size="sm" className={isCorporate ? 'rounded-xl bg-blue-600 font-bold text-white shadow-sm hover:bg-blue-700' : 'rounded-xl bg-indigo-600 font-bold text-white shadow-sm hover:bg-indigo-700'} />}>
      <ReceiptText className="mr-1.5 h-4 w-4" />{isCorporate ? 'Post corporate payment' : 'Settle invoice'}
    </DialogTrigger>
      ...
      <form onSubmit={submit}>
        <DialogHeader className="border-b border-slate-200 bg-white px-6 pb-5 pt-6">
          <div className="flex items-start gap-3">
            ...
            <div>
              <DialogTitle className="text-xl font-bold text-slate-900">{isCorporate ? 'Post corporate account payment' : 'Settle walk-out invoice'}</DialogTitle>
              <DialogDescription className="mt-1 text-sm leading-5 text-slate-500">{isCorporate ? 'Record a payment against this corporate account. The system applies it FIFO to open invoices.' : 'Record an individual payment against this walk-out invoice.'}</DialogDescription>
            </div>
          </div>
          <div className={`mt-5 flex items-center justify-between rounded-2xl border px-4 py-3 ${isCorporate ? 'border-blue-200 bg-blue-50' : 'border-indigo-200 bg-indigo-50'}`}>
            ...
            <div className="text-right"><p className="text-xs font-bold uppercase tracking-wide text-slate-500">Outstanding</p><p className={`mt-1 text-lg font-extrabold ${isCorporate ? 'text-blue-700' : 'text-indigo-700'}`}>{entry.currency || 'NGN'} {Number(entry.outstandingAmount).toLocaleString()}</p></div>
          </div>
        </DialogHeader>
