    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="mt-1 text-muted-foreground">Trace every inflow and outflow to its shift, guest, room, folio, and payment reference.</p>
        </div>
      ...
      {query.isLoading ? <div className="flex justify-center p-12"><Loader2 className="h-8 w-8 animate-spin text-primary" /></div> : query.error ? (
        ...
        <Card><CardHeader><CardTitle>Transaction detail</CardTitle></CardHeader><CardContent className="overflow-x-auto p-0">
          <table className="w-full min-w-[1250px] text-sm"><thead className="border-y bg-muted/40 text-left text-xs uppercase text-muted-foreground"><tr>{['Date', 'Direction', 'Type / description', 'Guest / room', 'Folio / reservation', 'Method', 'Shift', 'Amount'].map((heading) => <th key={heading} className="px-4 py-3">{heading}</th>)}</tr></thead>
            <tbody>{rows.map((row) => <tr key={`${row.kind}-${row.id}`} className="border-b last:border-0">
            ...
            </tr>)}{!rows.length && <tr><td colSpan={8} className="p-12 text-center text-muted-foreground">No frontdesk transactions found for this date range.</td></tr>}</tbody>
          </table>
        </CardContent></Card>
      </>}
    </div>
