...
import { Printer, Plus, Trash2, Wifi, CheckCircle, XCircle, Loader2, Settings2, ArrowLeft } from 'lucide-react';
...
function PrinterForm({
  ...
  useEffect(() => {
    if (form.connectionType !== 'NETWORK') discoverPrinters();
  }, [form.connectionType]);
      ...
      <div className="w-full max-w-lg bg-white dark:bg-[#1a1a1a] rounded-2xl shadow-2xl overflow-hidden">
        ...
        <div className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
          <div>
            ...
            <input value={form.name} onChange={(e) => set('name', e.target.value)} placeholder="e.g. Bar Receipt Printer"
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-2">
              {ROLES.map((r) => (
                <button key={r.value} type="button" onClick={() => set('printerRole', r.value)}
                  className={`p-3 rounded-xl border text-left transition-all ${form.printerRole === r.value ? 'border-indigo-500 bg-indigo-50 dark:bg-indigo-900/20' : 'border-gray-200 dark:border-[#2a2a2a] hover:border-gray-300'}`}>
          ...
          <div>
            ...
            <div className="flex gap-2">
              {['NETWORK', 'USB', 'SERIAL'].map(type => (
                <button key={type} type="button" onClick={() => set('connectionType', type)}
                  className={`flex-1 py-2.5 rounded-xl border text-xs font-medium transition-all ${form.connectionType === type ? 'border-indigo-500 bg-indigo-50 dark:bg-indigo-900/20 text-indigo-600 dark:text-indigo-400' : 'border-gray-200 dark:border-[#2a2a2a] text-gray-600 dark:text-gray-400'}`}>
                  {type}
                </button>
              ))}
            </div>
          ...
          {form.connectionType !== 'NETWORK' && (
            <div className="space-y-3">
              ...
              {availablePorts.length > 0 ? (
                <select value={form.devicePath} onChange={(e) => set('devicePath', e.target.value)}
                  className="w-full px-3 py-2.5 bg-gray-50 dark:bg-[#141414] border border-gray-200 dark:border-[#2a2a2a] rounded-xl text-sm outline-none focus:ring-2 focus:ring-indigo-500">
              ...
              {form.connectionType === 'SERIAL' && (
                <div>
                  ...
                  <input type="number" value={form.baudRate} onChange={(e) => set('baudRate', Number(e.target.value))}
                    className="w-full px-3 py-2.5 bg-gray-50 dark:bg-[#141414] border border-gray-200 dark:border-[#2a2a2a] rounded-xl text-sm font-mono outline-none focus:ring-2 focus:ring-indigo-500" />
                </div>
              )}
            </div>
          ...
          <div>
            ...
            <div className="flex gap-2">
              {PAPER_WIDTHS.map((w) => (
                <button key={w.value} type="button" onClick={() => set('paperWidth', w.value)}
                  className={`flex-1 py-2.5 rounded-xl border text-xs font-medium transition-all ${form.paperWidth === w.value ? 'border-indigo-500 bg-indigo-50 dark:bg-indigo-900/20 text-indigo-600 dark:text-indigo-400' : 'border-gray-200 dark:border-[#2a2a2a] text-gray-600 dark:text-gray-400'}`}>
                  {w.label}
                </button>
              ))}
            </div>
          ...
          <div>
            ...
            <input value={form.hotelName} onChange={(e) => set('hotelName', e.target.value)} placeholder="Grand Palace Hotel"
              className="w-full px-3 py-2.5 bg-gray-50 dark:bg-[#141414] border border-gray-200 dark:border-[#2a2a2a] rounded-xl text-sm outline-none focus:ring-2 focus:ring-indigo-500" />
          </div>

          <div>
            <label className="block text-xs font-semibold text-gray-500 dark:text-gray-400 mb-1">Address Line (printed on header)</label>
            <input value={form.hotelAddress} onChange={(e) => set('hotelAddress', e.target.value)} placeholder="123 Victoria Island, Lagos"
              className="w-full px-3 py-2.5 bg-gray-50 dark:bg-[#141414] border border-gray-200 dark:border-[#2a2a2a] rounded-xl text-sm outline-none focus:ring-2 focus:ring-indigo-500" />
          </div>
          ...
          {form.printerRole === 'RECEIPT' && (
            <label className="flex items-center justify-between p-3.5 bg-gray-50 dark:bg-[#141414] border border-gray-200 dark:border-[#2a2a2a] rounded-xl cursor-pointer">
              ...
              <div onClick={() => set('openCashDrawer', !form.openCashDrawer)}
...
// ─── Main View (used by both page route and PosApp overlay) ─────────────────
      ...
      <div className="bg-white dark:bg-[#111] border-b border-gray-100 dark:border-[#1f1f1f] px-6 py-5">
        <div className="max-w-4xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-4">
            {onClose ? (
              <button type="button" onClick={onClose} aria-label="Back to POS"
              ...
              <Link href="/pos" className="w-10 h-10 bg-gray-50 hover:bg-gray-100 dark:bg-[#1a1a1a] dark:hover:bg-[#252525] text-gray-500 dark:text-gray-400 rounded-xl flex items-center justify-center transition-colors">
                <ArrowLeft className="w-5 h-5" />
              </Link>
            )}
      ...
      <div className="max-w-4xl mx-auto px-6 py-8 space-y-6">
        ...
        {loading ? (
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {[1, 2].map((n) => <div key={n} className="h-52 bg-gray-100 dark:bg-[#1a1a1a] rounded-2xl animate-pulse" />)}
          </div>
        ) : printers.length === 0 ? (
