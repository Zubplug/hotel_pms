...
export function ApprovalControlCenter({ audience }: ApprovalCenterProps) {
  ...
  const act = async (action: 'approve' | 'reject' | 'issue') => {
    ...
    try {
      ...
      if (selected.kind === 'EVENT_INVOICE') { url = `/api/v1/approval-center/event-invoices/${selected.value.id}`; body = { action, discountAmount: discount ? Number(discount) : undefined, discountReason: reason, lineDiscounts: Object.fromEntries(Object.entries(lineDiscounts).map(([id, value]) => [id, Number(value || 0)])), lineReasons }; }
