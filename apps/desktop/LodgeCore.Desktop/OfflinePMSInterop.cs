...
public class OfflinePMSInterop
{
    ...
    public async Task<string> GetEventHallsAsync(string propertyId)
    {
        try { return JsonSerializer.Serialize(new { success = true, data = await _repo.GetEventHallsAsync(propertyId) }, _jsonOptions); }
        catch (Exception ex) { return JsonSerializer.Serialize(new { success = false, error = ex.Message }, _jsonOptions); }
    }
    ...
    public async Task<string> ProcessCheckInAsync(string reservationId, bool bypassKeycard = false, string encodedRoomId = "", string? encodeData = null, bool overrideDeposit = false, string? acknowledgedByStaffId = null, string? reason = null, string? operationId = null)
    {
        try
        {
            var ctx = await GetSecureContextAsync();
            ...
            var success = await _repo.ProcessCheckInAsync(reservationId, ctx.UserId, ctx.DeviceId, encodeData, overrideDeposit, acknowledgedByStaffId, reason, operationId);
            return JsonSerializer.Serialize(new { success }, _jsonOptions);
        }
        catch (Exception ex)
        {
    ...
    public async Task<string> PrintLaundryTicketAsync(string ticketDataJson)
    {
        ...
        {
            var ctx = await GetSecureContextAsync();
            await _repo.LogHardwareEventAsync(ctx.UserId, ctx.DeviceId, "LAUNDRY_TICKET_PRINT", ticketDataJson);
            ...
            var (success, error) = await _escPos.PrintLaundryTicketAsync(ticket, ctx.OutletId);
            return JsonSerializer.Serialize(new { success, error }, _jsonOptions);
        }
        catch (Exception ex)
        {
