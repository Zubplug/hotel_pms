...
with open('/Users/mac/hotel_pms/apps/web/src/lib/night-audit.ts', 'r') as f:
    content = f.read()

# 1. Stale IN_PROGRESS / POSTING recovery
old_stale_check = """    if (existing?.status === 'IN_PROGRESS' || existing?.status === 'POSTING') {
      throw new Error('CONFLICT:Night Audit is already in progress for this business date.');
    }"""

new_stale_check = """    if (existing?.status === 'IN_PROGRESS' || existing?.status === 'POSTING') {
...
# 2. Housekeeping Task Idempotency
old_hk_check = """            const existingTask = await tx.housekeepingTask.findFirst({
              where: {
                propertyId,
                roomId: room.id,
...
new_hk_check = """            const existingTask = await tx.housekeepingTask.findUnique({
              where: { idempotencyKey: hkIdempotencyKey }
            });"""

content = content.replace(old_hk_check, new_hk_check)

# 3. Heartbeat
old_batch_start = """    for (let i = 0; i < eligibleReservations.length; i += BATCH_SIZE) {
    const batch = eligibleReservations.slice(i, i + BATCH_SIZE);
    
    await Promise.all(batch.map(async (reservation: any) => {"""

new_batch_start = """    let lastHeartbeat = Date.now();
...
# 4. Final transaction wrapper
# Find where the final phase begins.
# It starts around `const journalPosting = await prisma.$transaction((tx) => postNightAuditJournal(tx, {`

# The final commit is currently:
# const [completedAudit] = await prisma.$transaction([
#  prisma.nightAudit.update({ ... })
#  ...
# ])

# We need to rewrite from `const journalPosting = ` to the end of the `$transaction([...])`.
# We'll use a regex to replace everything from `const journalPosting` to `return {` with our new interactive transaction.

# But wait, we also have to move `roomStatusCounts` into the transaction or before it. `roomStatusCounts` is currently before `const [completedAudit] =`.

final_phase_start_pattern = r"  const journalPosting = await prisma\.\$transaction\(\(tx\) => postNightAuditJournal\(\s*tx,\s*\{\s*propertyId,\s*businessDate: journalBusinessDate,\s*auditId: auditRun\.id,\s*createdBy: userId,\s*\}\)\);"

final_phase_end_pattern = r"  \]\);\s*return \{"

import re

# Match the old logic
match = re.search(final_phase_start_pattern + r".*?" + final_phase_end_pattern, content, re.DOTALL)
if not match:
    print("Could not find final phase!")
    exit(1)

old_final_phase = match.group(0)

# Replace prisma. with tx. in the old final phase where appropriate
# Wait, it's easier to just reconstruct the final phase
new_final_phase = """  const [completedAudit] = await prisma.$transaction(async (tx) => {
    let txErrors = errors;
    ...
    const departmentReconciliation = [
      ...
      { department: 'Other', source: schemaOtherRevenue, gl: glCredit('4400') },
    ...
    const completedRun = await tx.nightAudit.update({
      where: { id: auditRun.id },
      data: {
        status: 'COMPLETED',
    ...
    return [completedRun, txErrors];
  });
  
  errors = finalErrors;
  return {"""

content = content.replace(old_final_phase, new_final_phase)

# Also fix the variable reference `errors` being redefined inside the transaction
# Wait, I mapped it to `txErrors` in the script.
