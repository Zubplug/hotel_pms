...
enum ERROR_TYPE
{
    ...
    ERR_CHANGE_CARD     =   -32,      // 请换卡
    ERR_MASTER_FIRST    =   -33,      // 先做主卡， 才能做副卡（做副卡失败）
};
...
enum CARD_FLAGS
{
    ...
    CF_DOUBLE_MODE      =   0x04,      // DJE澳洲的double模式, 与宾客卡的亲友房时间戳标志冲突了.20191024把宾客卡的挪到bit6 !!!!!!
    CF_CHECK_TIMESTAMP  =   0x40,      // 亲友房更新和检测时间戳(宾客卡), 借用早期的主卡标志	
	...
	CF_PROHIBIT_DATA_EN =   0x10,     // 员工卡的禁入数据	
    CF_REPLACE_EN       =   0x40,     // 顶替使能(员工卡)
    CF_AUTO_LOST_EN     =   0x80,     // 自动挂失卡片(员工卡)

    CF_CHECKOUT_GUEST_ONLY= 0x10,     // 只退宾客卡
    CF_CHECKOUT_STAFF_ONLY= 0x20,     // 只退多门卡
    ...
    CF_SET_CHANNEL_TIME = (0x80 << 16),   // 设置通道门时段
};
...
enum MAKE_CARD_TYPE 
{
    ...
    OFFICE_CARD         =   15,     // 办公卡
    CLEAR_CARD          =   16,     // 清除卡(原管理卡) 20200526:MANAGE_CARD-->CLEAR_CARD
    STAFF_CARD          =   17,      // 员工卡
    ...
    ACS_ELEVATOR_SET_CARD = 24,     // 门禁电梯设置卡
	LOST_READ_CARD		=   27,		// 挂失提取卡
	BATCH_INFO_SET_CARD =   28,		// 批次信息设置卡
	BATCH_INFO_READ_CARD=   29,		// 批次信息读取卡
};
...
struct CARD_INFO{
    ...
    char    cSummerTimeStart[20];   // 夏令时起始
    char    cSummerTimeEnd[20];     // 夏令时结束: 如果月份>12 (减去16就得到实际的月份), 则表示下次遇到这个时段时不调整    
    char    cStaffProhibitedDays[200]; // 员工卡禁入的日期(从SDateTime的日期开始, SDateTime算第一天)
	BYTE    ClrReplaceOfCardType;	// 清除卡清除指定卡类型的替代号
	union {
		char	cLostSnList[433];		// 挂失卡号列表48*9=1(其它卡片如果结构体容量不够,可以考虑复用这个区域)
		char	cBatchInfo[433];		// 批次信息
	};

    char    cRFU2[26];              // 预留

    int     iRefreshCnt;            // RFU1 预留, 如果本次卡片信息与上次不同, 则iRefreshCnt就会变动, 否则保持不变
    int     iStaffProhibitedDayOfWeek; // 员工卡一周内的禁入设置(多门卡)
    int     iLostSnCnt;              // 挂失/取消挂失卡数量
    int     RFU4;                   // 预留   
};
...
struct LOCK_INFO{
    ...
    char    StopTime[20];           // 宾客卡终止时间/时间戳:    字符串, 19个字符
    ...
    char    cSummerTimeStart[20];   // 夏令时起始
    char    cSummerTimeEnd[20];     // 夏令时结束: 如果月份>12 (减去16就得到实际的月份), 则表示下次遇到这个时段时不调整  
    char    StartTime1[9];          // 通道门时段: 起始时间1: 字符串, 8个字符, 格式 hh:mm:ss
    char    EndTime1[9];            // 结束时间1:       字符串, 8个字符, 格式 hh:mm:ss
    char    StartTime2[9];          // 起始时间2:       字符串, 8个字符, 格式 hh:mm:ss
    char    EndTime2[9];            // 结束时间2:       字符串, 8个字符, 格式 hh:mm:ss
    char    StartTime3[9];          // 起始时间3:       字符串, 8个字符, 格式 hh:mm:ss
    ...
    char    cSysPara[40];           // 系统参数, 可用参数设置卡来设置
    char    StaffTimestamp[20];     // 多门卡时间戳:    字符串, 19个字符
    char    cRFU1[46];              // 预留
    char    cRFU2[200];             // 预留
    char    cRFU3[200];             // 预留
    char    cRFU4[199];             // 预留
    char    lockInfoVersion;        // 锁体信息版本
...
// 门禁/参数设置卡 数据结构
struct ACS_ELEVATOR_SET{
    UINT    forbidCardType;         // 楼栋号
    char    StartTime1[9];          // 起始时间2:       字符串, 8个字符, 格式 hh:mm:ss
    char    EndTime1[9];            // 结束时间1:       字符串, 8个字符, 格式 hh:mm:ss
    char    StartTime2[9];          // 起始时间2:       字符串, 8个字符, 格式 hh:mm:ss
    char    EndTime2[9];            // 结束时间2:       字符串, 8个字符, 格式 hh:mm:ss
...
typedef struct
{
    ...
    u8 BY53;
    u8 BY54;
} SYS_PARA_SETTING, *PSYS_PARA_SETTING;


// 位掩码
enum _BIT_MASK 
{
	BIT_MASK0    = ((UINT32)1<<0),
	BIT_MASK1    = ((UINT32)1<<1),
	BIT_MASK2    = ((UINT32)1<<2),
	BIT_MASK3    = ((UINT32)1<<3),
	BIT_MASK4    = ((UINT32)1<<4),
	BIT_MASK5    = ((UINT32)1<<5),
	BIT_MASK6    = ((UINT32)1<<6),
	BIT_MASK7    = ((UINT32)1<<7),
	BIT_MASK8    = ((UINT32)1<<8),
	BIT_MASK9    = ((UINT32)1<<9),
	BIT_MASK10   = ((UINT32)1<<10),
	BIT_MASK11   = ((UINT32)1<<11),
	BIT_MASK12   = ((UINT32)1<<12),
	BIT_MASK13   = ((UINT32)1<<13),
	BIT_MASK14   = ((UINT32)1<<14),
	BIT_MASK15   = ((UINT32)1<<15)
};
